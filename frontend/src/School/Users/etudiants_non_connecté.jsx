...
function EtudiantsNonConnecté() {
  ...
  const fetchUsers = async () => {
    const response = await axios.get("http://localhost:5001/users_non_connected");
    setUsers(response.data);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const user = { name: nom, prenom, email, password };

    try {
      if (editingUserId) {
        await axios.put(`http://localhost:5001/users_non_connected/${editingUserId}`, user);
        setEditingUserId(null);
      } else {
        await axios.post("http://localhost:5001/users_non_connected", user);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5001/users_non_connected/${id}`);
      fetchUsers();
    } catch (error) {
  ...
  return (
    <div style={styles.page}>
      <h1 style={styles.header}>Gestion des Utilisateurs Non Connectés</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <button
            ...
            style={{
              ...styles.btnSubmit,
              backgroundColor: editingUserId ? "#e67e22" : "#27ae60",
            }}
          >
...
background:
  ...
  btnAdd: {
    ...
    borderRadius: 50,
    cursor: "pointer",
    boxShadow: "0 8px 20px rgba(151, 169, 143, 0.42)",
...
export default EtudiantsNonConnecté;
