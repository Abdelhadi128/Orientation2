...
app.post("/api/users/import", async (req, res) => {
 ...
 app.post('/api/check-login', async (req, res) => {
  try {
    ...
    if (existingUser) {
        return res.json({ message: 'Déjà connecté !', redirect: `/user/${existingUser._id}`, userData: existingUser });
    }
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "École enregistrée avec succès", id: newSchool._id });
    } catch (error) {
